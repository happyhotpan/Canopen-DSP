...
/*邱家齐增加了SDO_BLOCK_SIZE的定义*/
#define SDO_BLOCK_SIZE  10
...
UNS8 lineToSDO (CO_Data* d, UNS8 line, UNS32 nbBytes, UNS8* data) {
	...
	//d->transfers[line].count = d->transfers[line].count*2;   //此行为新添行，为了使程序适应TI DSP char为16位   by hotpan
	...
	for (i = 0 ; i < nbBytes ; i++)
	{
		...
		UNS16 temp;
		if(i%2 == 0){
		temp = d->transfers[line].data[offset + j];
		* (data + i) = temp<<8>>8;} else
		{
		temp = d->transfers[line].data[offset + j];
		* (data + i) = temp>>8;}
	}
		...
		case 2:
			...
			if (whoami == SDO_SERVER) {
				...
				if (nbBytes > 4) {
					...
					err = lineToSDO(d, line, nbBytes, data + 4);   //DSP char 为16位 ，所以改写此函数
					if (err) {
						failedSDO(d, CliServNbr, whoami, index, subIndex, SDOABT_GENERAL_ERROR);
						return 0xFF;
					}
