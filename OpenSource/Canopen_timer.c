	...
	for(row_number=0, row=timers; row_number <= last_timer_raw + 1 && row_number < MAX_NB_TIMER; row_number++, row++)
	{
		...
		{	/* just store */
			...
			if (total_sleep_time > elapsed_time && total_sleep_time - elapsed_time > real_timer_value)
			{
				...
				Last_Begin_Time =0xFFFF - real_timer_value;     //记录下TIMER初试设定值，供getElapsedTime()使用
			}
...
void TimeDispatch(void)
{
	...
	/* 由于计数器是向下计数的,故需要修改下面一行	*/
	//Last_Begin_Time = 0xFFFF - next_wakeup;     //记录下TIMER初试设定值，供getElapsedTime()使用
	Last_Begin_Time = next_wakeup;     //记录下TIMER初试设定值，供getElapsedTime()使用
